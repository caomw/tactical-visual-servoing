...
static image<rgb> *loadIplImageFromMemory(IplImage *temp) 
{
	char buf[BUF_SIZE], doc[BUF_SIZE];
  
	/* load data */
	image<rgb> *im = new image<rgb>(temp->width, temp->height, true);
	int counter = 0;
	for (int y = 0; y < temp->height; y++) {
		for (int x = 0; x < temp->width; x++) {
			imRef(im, x, y).r = ((uchar*)(temp->imageData + temp->widthStep*y))[x*3];
			imRef(im, x, y).g = ((uchar*)(temp->imageData + temp->widthStep*y))[x*3+1];
			imRef(im, x, y).b = ((uchar*)(temp->imageData + temp->widthStep*y))[x*3+2];
...
static image<rgb> *loadIplImageFromFile(const char *name) 
{
	char buf[BUF_SIZE], doc[BUF_SIZE];
